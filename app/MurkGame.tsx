...
const postFragment = /* glsl */ `
  ...
  void main() {
    ...
    float chroma = (.16 + foundFootage * (.62 + r2 * 1.8)) * texel.x;
    vec3 col;
    ...
    float scanline = sin((gl_FragCoord.y + time * 5.4) * 1.54) * (.001 + foundFootage * .009);
    float tracking = smoothstep(.985, 1.0, sin(vUv.y * 39.0 - time * .46) * .5 + .5);
    float edgeWet = smoothstep(.62, 1.05, length(centered * vec2(.72, 1.0)));
    col += grain * (.008 + foundFootage * .029) + scanline + drop * .032 + haze;
    col *= 1.0 - tracking * (.004 + foundFootage * .026);
    float exposureFlutter = 1.0 + foundFootage * (sin(time * 3.7) * .008 + sin(time * .83) * .014);
    col *= exposureFlutter;
    col = pow(max(col, 0.0), vec3(mix(1.0, .94, foundFootage)));
    col *= mix(vec3(1.0), vec3(.94, 1.018, .972), foundFootage);
    ...
    float skyPixel = step(.9997, depthSample);
...
export default function MurkGame() {
  ...
  useEffect(() => {
    ...
    if (!directAudit) camera.position.set(0, 2.18, BOAT_Z - 2.65);
    if (auditHide) {
    ...
    const floodLight = new THREE.SpotLight(0xffe8c0, 0, 88, Math.PI * .205, .9, 1.46);
    ...
    const floodBeamGeo = new THREE.ConeGeometry(16.2, 68, 64, 1, true);
    floodBeamGeo.rotateX(Math.PI / 2);
    floodBeamGeo.translate(.44, -.22, -34);
    const floodBeamMaterial = new THREE.ShaderMaterial({
      transparent: true,
      depthWrite: false,
      side: THREE.DoubleSide,
      blending: THREE.AdditiveBlending,
      ...
      fragmentShader: /* glsl */ `
        ...
        void main() {
          float depth = clamp((-vLocal.z) / 68.0, 0.0, 1.0);
          float expectedRadius = max(.18, depth * 16.2);
          float radial = length(vLocal.xy) / expectedRadius;
          float feather = 1.0 - smoothstep(.28, 1.0, radial);
          float longitudinal = smoothstep(.01, .08, depth) * (1.0 - smoothstep(.68, 1.0, depth));
          float particulate = .92 + hash(floor(vLocal.xy * 2.2 + time * .35)) * .08;
          gl_FragColor = vec4(vec3(1.0, .79, .44), opacity * feather * longitudinal * particulate);
        }
      `,
    });
    const floodBeamVolume = new THREE.Mesh(floodBeamGeo, floodBeamMaterial);
    floodBeamVolume.renderOrder = 2;
    camera.add(floodBeamVolume);

    const floodRig = new THREE.Group();
    const floodHousingMaterial = new THREE.MeshStandardMaterial({ color: 0x272d27, roughness: .72, metalness: .38, flatShading: true });
    const floodTrimMaterial = new THREE.MeshStandardMaterial({ color: 0x777967, roughness: .42, metalness: .62, flatShading: true });
    const floodLensMaterial = new THREE.MeshBasicMaterial({ color: 0xffd58b, transparent: true, opacity: .82 });
    const floodHousing = new THREE.Mesh(new THREE.BoxGeometry(.76, .43, .42), floodHousingMaterial);
    const floodLens = new THREE.Mesh(new THREE.CylinderGeometry(.27, .31, .08, 10), floodLensMaterial);
    floodLens.rotation.x = Math.PI / 2;
    floodLens.position.z = -.24;
    const floodHandle = new THREE.Mesh(new THREE.TorusGeometry(.31, .055, 5, 10, Math.PI), floodTrimMaterial);
    floodHandle.rotation.z = Math.PI;
    floodHandle.position.y = .34;
    floodRig.add(floodHousing, floodLens, floodHandle);
    floodRig.position.set(.54, -.43, -.82);
    floodRig.rotation.set(-.13, -.18, -.08);
    ...
    mastLampLens.position.set(.78, 3.18, -.12);
    ...
    const shaftSpots = Array.from({ length: 4 }, (_, index) => [
      (rand() - .5) * 72,
      BOAT_Z - 6 - rand() * 58,
      .9 + rand() * 1.5,
      [.048, .036, .022, .012][index],
    ]);
    for (const [x, z, radius, opacity] of shaftSpots) {
      const geometry = new THREE.CylinderGeometry(radius * .16, radius, 24, 8, 1, true);
    ...
    function orientCreatureAlongTravel(dt: number) {
      ...
      creature.quaternion.slerp(creatureLookHelper.quaternion, 1 - Math.exp(-dt * 2.4));
    }
    ...
    function ensureAudio() {
      if (!audioContext) {
        ...
        // A quiet, procedural title drone: no external music license and no
        // obvious melody, just an uneasy low beating tone and distant wash.
        for (const [frequency, level] of [[36.5, .42], [39.2, .28]] as const) {
          const tone = audioContext.createOscillator();
          const toneGain = audioContext.createGain();
          tone.type = "sine";
          tone.frequency.value = frequency;
          toneGain.gain.value = level;
          tone.connect(toneGain).connect(introBus);
          tone.start();
        }
        const introBuffer = createNoiseBuffer(5.1);
        if (introBuffer) {
          const introNoise = audioContext.createBufferSource();
          const introFilter = audioContext.createBiquadFilter();
          const introNoiseGain = audioContext.createGain();
          introNoise.buffer = introBuffer;
          introNoise.loop = true;
          introFilter.type = "lowpass";
          introFilter.frequency.value = 94;
          introFilter.Q.value = 3.1;
          introNoiseGain.gain.value = .17;
          introNoise.connect(introFilter).connect(introNoiseGain).connect(introBus);
          introNoise.start();
        }
    ...
    function playCreatureClip(name: keyof typeof creatureAudioUrls, tier: CallTier, gainScale = 1) {
      ...
      gain.gain.value = { far: .62, medium: .82, close: 1 }[tier] * gainScale;
    ...
    function playCreatureCall(mood: CallMood, tier: CallTier) {
      const curiousChoices = ["favorite-non-aggro", "favorite-non-aggro", "random-1", "random-2"] as const;
      const huntingChoices = tier === "far"
        ? ["far-aggro-1", "far-aggro-2"] as const
        : ["mid-aggro"] as const;
      const choices = mood === "curious" ? curiousChoices : huntingChoices;
      const name = choices[Math.floor(rand() * choices.length)];
    ...
    function sendSonarPing(now = performance.now()) {
      ...
      const bearing = THREE.MathUtils.radToDeg(Math.atan2(
        beamDir.z * contactDirection.x - beamDir.x * contactDirection.z,
        beamDir.x * contactDirection.x + beamDir.z * contactDirection.z,
      ));
      const uncertainty = Math.min(22, 5 + distance * .09);
      setSonarBearing(Math.round(bearing + (rand() - .5) * uncertainty));
      setSonarMapPosition({
    ...
    function fireCamera(now = performance.now()) {
      if (!running || !cameraIsHeld || exposuresLeft <= 0 || now < flashPeakUntil) return;
      ensureAudio();
      ...
      if (photographed) {
        ...
        setPhotoCount(photographedEvidence.size);
    ...
    function onKeyDown(event: KeyboardEvent) {
      ...
      if (event.code === "KeyF" && !event.repeat && (running || onBoat)) {
        ...
        floodRig.visible = floodlightOn;
        setFloodOn(floodlightOn);
        if (floodlightOn) {
          cameraIsHeld = false;
          cameraRig.visible = false;
          setCameraHeld(false);
        ...
        playInstrumentClick(floodlightOn ? 1.12 : .72);
      }
    ...
    function animate(now: number) {
      ...
      (scene.fog as THREE.FogExp2).density = THREE.MathUtils.lerp(underwaterFogDensity, .064, surfaceBlend);
      ...
      hemi.intensity = THREE.MathUtils.lerp(THREE.MathUtils.lerp(.29, .055, depth01), 1.05, surfaceBlend);
      daylight.intensity = THREE.MathUtils.lerp(0, 2.35, surfaceBlend);
      const targetExposure = THREE.MathUtils.lerp(THREE.MathUtils.lerp(.92, .7, depth01), 1.08, surfaceBlend);
      ...
      shaftMaterials.forEach((material, index) => {
        material.opacity = ([.055, .044, .026, .014][index] ?? .012) * shaftStrength;
      });
      ...
      if (running) {
        ...
        if (breathingAtSurface) {
          ...
          airValue -= dt * (.15 + breathDebtValue * .46);
      ...
      if (audioContext && waterFilter && waterBus) {
        ...
        if (introBus) {
          const introLevel = !sessionStarted ? .12 : onBoat ? .08 : .0001;
        ...
        const waterLevel = authoredSilence ? .0001 : THREE.MathUtils.lerp(.16, .26, depth01);
      ...
      if (running) {
        ...
        if (rising) move.y += 1;
        if (descending) move.y -= 1;
        if (move.lengthSq() > 0) move.normalize();

        const speed = grounded && move.y <= 0 ? 1.7 : 2.55;
        const swimResponse = now < boostCooldownUntil ? .72 : 3.2;
        velocity.lerp(move.multiplyScalar(speed), 1 - Math.exp(-dt * swimResponse));
      ...
      if (!activeSilhouettePass && openingSeclusionActive) {
      ...
      if (!activeSilhouettePass && travelDistance > desiredDistance + .2) {
        moveCreatureSafely(creatureTarget, creatureSpeed, dt);
      ...
      const gearActive = running || onBoat;
      diverLight.intensity = gearActive && mainLampOn ? 190 : 0;
      pilotLight.intensity = gearActive && !mainLampOn ? 26 : 0;
      beamMaterial.opacity = gearActive && mainLampOn ? .072 : 0;
      coreMaterial.opacity = gearActive ? (mainLampOn ? .065 : .018) : 0;
      right.crossVectors(beamDir, WORLD_UP).normalize();
      floodLight.position.copy(camera.position).addScaledVector(right, .42).addScaledVector(WORLD_UP, -.24);
      floodTarget.position.copy(camera.position).addScaledVector(beamDir, 36);
      floodLight.intensity = gearActive && floodlightOn ? 930 : 0;
      floodBeamMaterial.uniforms.opacity.value = gearActive && floodlightOn ? .155 : 0;
      floodBeamMaterial.uniforms.time.value = now / 1000;
      floodRig.position.y = -.43 + Math.sin(now * .0017) * .012;
      cameraRig.position.y = -.5 + Math.sin(now * .00145) * .01;
      sonarRig.visible = gearActive && now < sonarVisibleUntil;
      ...
      for (let i = 0; i < scatterCount; i++) {
        let particleY = scatterAttribute.getY(i) - velocity.y * dt;
        if (particleY > 7) particleY = -7;
        if (particleY < -7) particleY = 7;
      ...
      postMaterial.uniforms.foundFootage.value = foundFootageRef.current ? 1 : 0;
      renderer.setRenderTarget(target);
      renderer.render(scene, camera);
      renderer.setRenderTarget(null);
      renderer.render(postScene, postCamera);
  ...
  return (
    <main className="game-shell" aria-label="MURK underwater survival game">
      ...
      {started && (
        <div className={`helmet-shell ${lampOn ? "lamp-powered" : ""}`} aria-hidden="true">
          <span className="helmet-lamp-bulb" />
        </div>
      )}
      {started && !fatalCause && divePhase !== "complete" && (
        <div className="hud" aria-hidden="true">
          ...
          {foundFootage && (
            <div className="found-footage-frame">
              <span>● REC</span><b>HI8 · AUTO</b>
            </div>
          )}
          {divePhase !== "onboat" && (
            <>
              ...
              <div className="lamp-stack">
                <div className={`lamp-readout ${lampOn ? "is-on" : "is-off"}`}><span className="lamp-dot" /></div>
                <div className={`lamp-readout ${floodOn ? "is-on" : "is-off"}`}><span className="lamp-dot" /></div>
              </div>
          ...
          {mapHeld && (divePhase === "search" || divePhase === "onboat") && (
            <section className="field-map">
              ...
              <div className="map-lake">
                ...
                <span className="contour-label label-05">05</span><span className="contour-label label-12">12</span>
                <span className="contour-label label-24">24</span><span className="contour-label label-40">40m</span>
                <span className="deep-end-contours"><i /><i /><i /><b>DEEP END</b></span>
                <span className="map-poi point-boat"><i className="poi-sketch sketch-boat"><u /><u /><u /></i><b>ENTRY</b></span>
                <span className="map-poi point-forest"><i className="poi-sketch sketch-forest"><u /><u /><u /></i><b>DROWNED FOREST</b></span>
                <span className="map-poi point-road"><i className="poi-sketch sketch-road"><u /><u /></i><b>OLD ROAD</b></span>
                <span className="map-poi point-pillars"><i className="poi-sketch sketch-pillars"><u /><u /><u /></i><b>SPIRES</b></span>
                <span className="map-poi point-church"><i className="poi-sketch sketch-church"><u /><u /></i><b>CHURCH</b></span>
      ...
      {!started && (
        <section className="start-screen" onPointerDown={() => introAudioRef.current()}>
          <div className="start-lake-surface" aria-hidden="true">
            <i className="title-fog fog-near" /><i className="title-fog fog-far" />
            <i className="title-shore shore-left" /><i className="title-shore shore-right" />
            <i className="title-boat">
              <i className="title-boat-hull" />
              <i className="title-boat-deck" />
              <i className="title-boat-cabin"><i className="title-boat-window" /></i>
              <i className="title-boat-roof" />
              <i className="title-boat-mast"><i className="title-boat-lamp" /></i>
              <i className="title-boat-rail rail-a" /><i className="title-boat-rail rail-b" />
            </i>
            <i className="title-water-line" />
            <i className="title-ripple ripple-one" /><i className="title-ripple ripple-two" /><i className="title-ripple ripple-three" />
          </div>
          <div className="start-card">
            <h1 className="start-title">MURK</h1>
            <p className="start-copy"><strong>There’s something in Greenwake Lake.</strong></p>
            <button className="dive-button" onClick={begin}>JUMP IN</button>
          </div>
        </section>
      )}
      {fatalCause && (
      ...
      {divePhase === "complete" && !fatalCause && (
        <section className="complete-screen" aria-live="polite">
          ...
          <span>{carryLabels.length} PHYSICAL · {photoCount} EVIDENCE PHOTOS</span>
      ...
      {started && pauseOpen && !fatalCause && divePhase !== "complete" && (
        <div className="pause-note">
          ...
          <div className="screen-actions"><button onClick={resumeDive}>CONTINUE</button><button onClick={restartDive}>RESTART</button><button onClick={goHome}>HOME</button></div>
          <button className="visual-mode-toggle" onClick={toggleFoundFootage}>FOUND FOOTAGE · {foundFootage ? "ON" : "OFF"}</button>
          <div className="pause-controls-list" aria-label="Full controls">
