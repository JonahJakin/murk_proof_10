...
export interface CreatureRig {
  ...
  /** Animate the articulated rig with elapsed time in seconds. */
...
export function createCreature(
  ...
  const update = (seconds: number) => {
    ...
    for (let index = 0; index < neckJoints.length; index++) {
      const phase = seconds * 1.18 - index * .37;
    ...
    for (let index = 0; index < tailJoints.length; index++) {
      const phase = seconds * 1.22 - index * .44;
    ...
    for (const flipper of flippers) {
      for (let index = 0; index < flipper.chain.length; index++) {
        const phase = seconds * 1.58 + Math.floor(flipper.index / 2) * 1.05 + index * .38;
    ...
    head.rotation.y = Math.sin(seconds * 1.13) * .07;
    head.rotation.x = Math.sin(seconds * .73 + .4) * .026;
  };
